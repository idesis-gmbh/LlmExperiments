...
# )
"""
TOOLS[0]["handler"] = lambda tool_call: get_sqlite_schema()
TOOLS[1]["handler"] = lambda tool_call: get_sqlite_tables()
TOOLS[2]["handler"] = lambda tool_call: get_sqlite_table(
    tool_call["function"]["arguments"]["table"]
)
TOOLS[3]["handler"] = lambda tool_call: query_sqlite(
    tool_call["function"]["arguments"]["query"],
    tool_call["function"]["arguments"]["parameters"]
    if "parameters" in tool_call["function"]["arguments"]
    else [],
)
...
def run_chat_stream(message, history):
    ...
    for event in chat_stream(messages, TOOLS):
        ...
        yield (
            response["thinking"],
            json.dumps(response["tooling"], indent=2),
            messages,
            "",
        )
